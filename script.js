...
// ===== FUNCIÓN PARA GENERAR COLORES DINÁMICAMENTE PARA DELITOS =====
function generateDelitoColors(delitos) {
    const coloresBase = [
        '#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#96CEB4', '#FFEAA7',
        '#DDA0DD', '#98D8C8', '#F7DC6F', '#BB8FCE', '#85C1E2', '#F8B88B',
        '#A9CCE3', '#F5B7B1', '#D7BDE2'
    ];
    
    const coloresDelitos = {};
    delitos.forEach((delito, index) => {
        coloresDelitos[delito] = coloresBase[index % coloresBase.length];
    });
...
class RealTimeDataManager {
    constructor() {
        this.googleSheetsUrl = 'https://docs.google.com/spreadsheets/d/1jWUZ_IjqmrYGlzPeMHBFI7cHX-CFulXRvPDffVNkfiY/export?format=csv&gid=0';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const downloadChartsScreenshot = document.getElementById('download-charts-screenshot');
    if (downloadChartsScreenshot) {
        downloadChartsScreenshot.addEventListener('click', function(e) {
            e.preventDefault();
            const chartsSection = document.getElementById('charts-section');
            html2canvas(chartsSection).then(canvas => {
                const link = document.createElement('a');
                link.download = 'captura_graficas.jpeg';
    ...
    const downloadAllCharts = document.getElementById('download-all-charts');
    if (downloadAllCharts) {
        downloadAllCharts.addEventListener('click', function(e) {
            e.preventDefault();
            const chartIds = ['municipioChart', 'anualChart', 'fosasChart', 'fosasAnualChart'];
            chartIds.forEach(id => {
    ...
    // --- Descarga de Mapa ---
    const downloadMapScreenshot = document.getElementById('download-map-screenshot');
    if (downloadMapScreenshot) {
        downloadMapScreenshot.addEventListener('click', function(e) {
            e.preventDefault();
            const mapSection = document.getElementById('map-section');
            html2canvas(mapSection).then(canvas => {
                const link = document.createElement('a');
                link.download = 'captura_mapa.jpeg';
                link.href = canvas.toDataURL('image/jpeg');
                link.click();
            });
        });
    ...
    // --- Descarga de Tabla ---
    const downloadTableScreenshot = document.getElementById('download-table-screenshot');
    if (downloadTableScreenshot) {
        downloadTableScreenshot.addEventListener('click', function(e) {
            e.preventDefault();
            const tableSection = document.getElementById('table-section');
            html2canvas(tableSection).then(canvas => {
                const link = document.createElement('a');
                link.download = 'captura_tabla.jpeg';
                link.href = canvas.toDataURL('image/jpeg');
                link.click();
            });
        });
    ...
    const downloadTableXLSX = document.getElementById('download-table-xlsx');
    if (downloadTableXLSX) {
        downloadTableXLSX.addEventListener('click', function(e) {
            e.preventDefault();
            const pivotTable = document.getElementById('pivot-table');
            const wb = XLSX.utils.table_to_book(pivotTable, {sheet: 'Tabla Dinamica'});
